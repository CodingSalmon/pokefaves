...
import { getComments, createComment, removeComment } from '../../services/comment-api';
...
const PokemonPage = ({user, handleFavorite}) => {
    ...
    const addComment = async (e) => {
        ...
        form.reset();
        setState({...state, comments:[...state.comments, comment], formData: {msg:''}});
    }

    const deleteComment = async (id) => {
        await removeComment(id);
        setState({...state, comments: state.comments.filter(comment => comment._id !== id)})
    }
        ...
        <div className='PokemonPage'>
            ...
            <div>
                <div className='PokemonPage-image'>
                    <span><img height="250" src={state.pokemon.sprites.front_default} alt=""/></span>
                </div>

                <div className='content'>
                    <div className='PokemonPage-details'>
                        <span>Name:</span>
                        <span style={{textTransform: 'capitalize'}}>{state.pokemon.name}</span>
                        <span>Type(s):</span>
                        <span>
                        ...
                        <span>Height:</span>
                        <span>{Math.floor(state.pokemon.height / 3.048)} ft. {Math.ceil(12 * (state.pokemon.height / 3.048 - Math.floor(state.pokemon.height / 3.048)))} in.</span>
                        <span>Weight:</span>
                        ...
                        {user ? 
                            <span className='favs'>
                                {state.pokemon.types.map((type, idx) =>
                                    <button key={idx} className='fav btn' onClick={() => handleFavorite(type.type.name, state.pokemon)}>
                                        {(user.favorites[`${type.type.name}`] !== state.pokemon.name) ? `Make your favorite ${type.type.name} pokemon`: `Get rid of my favorite ${type.type.name} pokemon`}
                                    </button>
                                )}
                            </span>
                        :''}
                    ...
                    <div className='commentArea'>
                        {user ? 
                            <form className='commentForm' onSubmit={addComment}>
                                <h4>Add Comment</h4>
                                <input type="text" name='msg' onChange={handleChange} placeholder='Your text here'></input>
                        ...
                        {state.comments.length ? 
                            state.comments.filter(comment => comment.pokemonName === state.pokemon.name).map((comment, idx) => 
                                <div className='comment' key={idx}>
                                    ...
                                    {user && user._id === comment.postedBy ?
                                        <input type="submit" value='DELETE' className='btn' onClick={() => deleteComment(comment._id)}/>
                                    :''}
                                </div>
                            )
